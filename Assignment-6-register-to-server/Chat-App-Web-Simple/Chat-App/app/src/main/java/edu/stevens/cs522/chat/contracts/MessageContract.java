...
public class MessageContract extends BaseContract {
    ...
    public static int getMessageSequenceNumber(Cursor cursor){
        return cursor.getInt(cursor.getColumnIndexOrThrow(MESSAGE_SEQUENCE_NUMBER));
    }

    public static void putMessageSequenceNumber(ContentValues values, int messageSequenceNumber){
        values.put(MESSAGE_SEQUENCE_NUMBER, messageSequenceNumber);
    }
}
